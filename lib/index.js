...
const joiMethodMapper = new Map([
  // Array validation.
  ['arrayLength', { isArrayType: true, name: 'length' }],
  ...
  ['objectLength', { name: 'length' }],
  ['objectMax', { name: 'max' }],
  ['objectMin', { name: 'min' }],

  // String validation.
  ['case', { argumentMapper: argToLowercase }],
  ['length', { argumentMapper: argToStringLengthOptions }],
  ['maxLength', { name: 'max', argumentMapper: argToStringLengthOptions }],
  ['minLength', { name: 'min', argumentMapper: argToStringLengthOptions }],
...
function mapGqlToJoiType (gqlType, joiSchema, fieldName, dirArgs) {
  ...
  if (isListType(gqlType)) {
    const namedTypeArgs = dirArgs.filter((arg) => {
      ...
      return !(mapper && mapper.isArrayType);
    });
...
function applyDirectiveArguments (joiSchema, fieldName, dirArgs) {
  for (let i = 0; i < dirArgs.length; ++i) {
    ...
    if (mapper !== undefined) {
      const { name, argumentMapper, typeCast } = mapper;
