'use strict';
const directiveSchema = `
directive @validate(
  # Array validation.
  arrayLength: Int
  arrayMax: Int
  arrayMin: Int
  sort: ValidateSortInput
  unique: ValidateUniqueInput

  # Date validation.
  dateGreater: String
  iso: ValidateFlag
  dateLess: String
  dateMax: String
  dateMin: String
  timestamp: ValidateTimestampType

  # Number validation.
  greater: Float
  integer: ValidateFlag
  less: Float
  max: Float
  min: Float
  multiple: Float
  negative: ValidateFlag
  port: ValidateFlag
  positive: ValidateFlag
  precision: Int
  sign: ValidateSign
  unsafe: Boolean

  # Object validation.
  and: [String]
  objectLength: Int
  objectMax: Int
  objectMin: Int
  nand: [String]
  or: [String]
  oxor: [String]
  xor: [String]
  with: ValidateWithInput
  without: ValidateWithInput

  # String validation.
  alphanum: ValidateFlag
  base64: ValidateBase64Input
  case: ValidateCaseDirection
  creditCard: ValidateFlag
  dataUri: ValidateDataUriInput
  domain: ValidateDomainInput
  email: ValidateEmailInput
  guid: ValidateGuidInput
  hex: Boolean
  hostname: ValidateFlag
  ip: ValidateIpInput
  isoDate: ValidateFlag
  isoDuration: ValidateFlag
  length: ValidateStringLengthInput
  lowercase: ValidateFlag
  maxLength: ValidateStringLengthInput
  minLength: ValidateStringLengthInput
  normalize: ValidateNormalizeForm
  pattern: ValidatePatternInput
  regex: ValidatePatternInput
  token: ValidateFlag
  trim: Boolean
  uppercase: ValidateFlag
  uuid: ValidateGuidInput

  # Helpers.
  type: ValidateTypes
) on ARGUMENT_DEFINITION | INPUT_FIELD_DEFINITION

enum ValidateFlag { TRUE }
enum ValidateCaseDirection { UPPER LOWER }
enum ValidateGuidSeparator { NONE COLON DASH COLON_OR_DASH }
enum ValidateGuidVersion { UUIDV1 UUIDV2 UUIDV3 UUIDV4 UUIDV5 }
enum ValidateIpVersion { IPV4 IPV6 IPVFUTURE }
enum ValidateIpCidr { OPTIONAL REQUIRED FORBIDDEN }
enum ValidateNormalizeForm { NFC NFD NFKC NFKD }
enum ValidateSign { NEGATIVE POSITIVE }
enum ValidateSortOrder { ASCENDING DESCENDING }
enum ValidateTimestampType { UNIX JAVASCRIPT }
enum ValidateTypes { DATE }

input ValidateBase64Input {
  paddingRequired: Boolean
  urlSafe: Boolean
}

input ValidateDataUriInput {
  paddingRequired: Boolean
}

input ValidateDomainInput {
  allowUnicode: Boolean
  minDomainSegments: Int
  maxDomainSegments: Int
  # TODO(cjihrig): Support tlds option.
}

input ValidateEmailInput {
  allowUnicode: Boolean
  ignoreLength: Boolean
  minDomainSegments: Int
  maxDomainSegments: Int
  multiple: Boolean
  separator: String
  # TODO(cjihrig): Support tlds option.
}

input ValidateGuidInput {
  version: [ValidateGuidVersion]
  separator: ValidateGuidSeparator
}

input ValidateIpInput {
  version: [ValidateIpVersion]
  cidr: ValidateIpCidr
}

input ValidatePatternInput {
  pattern: String
  flags: String
  name: String
  invert: Boolean
}

input ValidateSortInput {
  order: ValidateSortOrder
  by: String
}

input ValidateStringLengthInput {
  limit: Int!
  encoding: String
}

input ValidateUniqueInput {
  comparator: String
}

input ValidateWithInput {
  key: String!
  peers: [String]!
}

input ValidateWithoutInput {
  key: String!
  peers: [String]!
}
`;

module.exports = { directiveSchema };
